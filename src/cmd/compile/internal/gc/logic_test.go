...
func TestLogic64(t *testing.T) {
	...
	zero := func x { return 0 }
	id := func x { return x }
	or := func x, y { return x | y }
	and := func x, y { return x & y }
	y := func x, y { return y }
	...
	for _, test := range [...]struct {
		...
		{"x|x", func x { return x | x }, id},
		{"x|0", func x { return x | 0 }, id},
		{"x|-1", func x { return x | -1 }, func x { return -1 }},
		{"x&x", func x { return x & x }, id},
		{"x&0", func x { return x & 0 }, zero},
		{"x&-1", func x { return x & -1 }, id},
		{"x^x", func x { return x ^ x }, zero},
		{"x^0", func x { return x ^ 0 }, id},
		{"x^-1", func x { return x ^ -1 }, func x { return ^x }},
		{"x+0", func x { return x + 0 }, id},
		{"x-x", func x { return x - x }, zero},
		{"x*0", func x { return x * 0 }, zero},
		{"^^x", func x { return ^^x }, id},
	} {
	...
	for _, test := range [...]struct {
		...
		{"x|(x|y)", func x, y { return x | (x | y) }, or},
		{"x|(y|x)", func x, y { return x | (y | x) }, or},
		{"(x|y)|x", func x, y { return (x | y) | x }, or},
		{"(y|x)|x", func x, y { return (y | x) | x }, or},
		{"x&(x&y)", func x, y { return x & (x & y) }, and},
		{"x&(y&x)", func x, y { return x & (y & x) }, and},
		{"(x&y)&x", func x, y { return (x & y) & x }, and},
		{"(y&x)&x", func x, y { return (y & x) & x }, and},
		{"x^(x^y)", func x, y { return x ^ (x ^ y) }, y},
		{"x^(y^x)", func x, y { return x ^ (y ^ x) }, y},
		{"(x^y)^x", func x, y { return (x ^ y) ^ x }, y},
		{"(y^x)^x", func x, y { return (y ^ x) ^ x }, y},
		{"-(y-x)", func x, y { return -(y - x) }, func x, y { return x - y }},
		{"(x+y)-x", func x, y { return (x + y) - x }, y},
		{"(y+x)-x", func x, y { return (y + x) - x }, y},
	} {
		for _, v := range values {
